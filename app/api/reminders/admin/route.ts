...
export async function GET() {
  ...
  try {
    ...
    const [{ count: activeCount, error: activeError }, { count: totalCount, error: totalError }, { data: subscribers, error: subscribersError }] =
      await Promise.all([
        supabaseAdmin
        ...
        supabaseAdmin
          .from('email_reminders')
          .select(
            'id, email, active, reminder_mode, scheduled_time_minutes, timezone, source_path, sent_count, last_sent_at, last_sent_on, created_at, updated_at'
          )
          .order('active', { ascending: false })
          .order('created_at', { ascending: false }),
      ])

    if (activeError || totalError || subscribersError) {
      return NextResponse.json(
        { error: 'Could not load reminder admin data.' },
        { status: 500 }
      )
    }
